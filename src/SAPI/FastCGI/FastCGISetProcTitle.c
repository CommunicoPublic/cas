...
void initproctitle(int argc, char **argv)
{
	 ...
	 */
	for (i = 0; aEnv[i] != NULL; i++) { continue; }

	environ = (char **) malloc(sizeof(char *) * (i + 1));
	if (environ == NULL) { return; }

	for (i = 0; aEnv[i] != NULL; i++)
	{
		if ((environ[i] = strdup(aEnv[i])) == NULL) { return; }
	}
	environ[i] = NULL;
	...
	if (i > 0)
	{
		argv_lth = aEnv[i-1] + strlen(aEnv[i-1]) - argv0[0];
	}
	else
	{
...
void setproctitle(const char *prog)
{
	...
	if (strlen(prog) + 5 > SPT_BUFSIZE) { return; }

	i = snprintf(buf, SPT_BUFSIZE, "%s", prog);
